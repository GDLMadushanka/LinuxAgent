...
/**
 * Contains utility methods used by linuxdevice plugin.
 */
public class LaptopUtils {

    private static Log log = LogFactory.getLog(LaptopUtils.class);
    ...
    public static void setupDeviceManagementSchema() throws DeviceTypeException {
        try {
            ...
            DataSource dataSource = (DataSource) ctx.lookup(LaptopConstants.DATA_SOURCE_NAME);
            LaptopSchemaInitializer initializer =
                    new LaptopSchemaInitializer(dataSource);
            log.info("Initializing device management repository database schema");
            initializer.createRegistryDatabase();
        } catch (NamingException e) {
            log.error("Error while looking up the data source: " + LaptopConstants.DATA_SOURCE_NAME);
        } catch (Exception e) {
